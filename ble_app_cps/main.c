...
#define BLE_UUID_CYCLING_POWER                     0x1818     /**< Cycling Speed and Cadence service UUID. */
#define POWER_MEAS_INTERVAL     APP_TIMER_TICKS(750, APP_TIMER_PRESCALER) /**< Battery level measurement interval (ticks). */

#define IS_SRVC_CHANGED_CHARACT_PRESENT 0                                          /**< Include or not the service_changed characteristic. if not enabled, the server's database cannot be changed for the lifetime of the device*/

#define DEVICE_NAME                     "Garmyn"                               /**< Name of device. Will be included in the advertising data. */
...
static ble_uuid_t m_adv_uuids[] = {{BLE_UUID_CYCLING_POWER,  BLE_UUID_TYPE_BLE},
...
static sensorsim_cfg_t                  m_power_sim_cfg;                        
static sensorsim_state_t                m_power_sim_state;                   

APP_TIMER_DEF(m_cps_meas_timer_id);   																 
			
static void services_init_cps(void)
{
    uint32_t              err_code;
    ...
    cps_init.evt_handler = NULL;
	  cps_init.ble_cps_sensor_location_initial_value.sensor_location.sensor_location = SENSOR_LOCATION_LEFT_CRANK;   // for the measurement characteristic, only the CCCD write permission can be set by the application, others are mandated by service specification
    cps_init.ble_cps_cycling_power_feature_initial_value.cycling_power_feature.accumulated_energy_supported = ACCUMULATED_ENERGY_SUPPORTED_FALSE;
		
	  cps_init.ble_cps_cycling_power_measurement_initial_value.accumulated_energy = 0;
		cps_init.ble_cps_cycling_power_feature_initial_value.cycling_power_feature.chain_length_adjustment_supported = CHAIN_LENGTH_ADJUSTMENT_SUPPORTED_FALSE;
		cps_init.ble_cps_cycling_power_feature_initial_value.cycling_power_feature.pedal_power_balance_supported = PEDAL_POWER_BALANCE_SUPPORTED_FALSE;
		cps_init.ble_cps_cycling_power_feature_initial_value.cycling_power_feature.crank_revolution_data_supported  = CRANK_REVOLUTION_DATA_SUPPORTED_FALSE;
...
{
	  m_power_sim_cfg.min          = POWER_MIN;
    m_power_sim_cfg.max          = POWER_MAX;
    m_power_sim_cfg.incr         = POWER_INCREMENT;
    m_power_sim_cfg.start_at_max = true;

    sensorsim_init(&m_power_sim_state, &m_power_sim_cfg);
...
static void cps_sim_measurement(ble_cps_cycling_power_measurement_t * p_measurement)
{
	  uint8_t  power;

    power = (uint8_t)sensorsim_measure(&m_power_sim_state, &m_power_sim_cfg);
	  		
		p_measurement->instantaneous_power = power;
}

static void cps_meas_timeout_handler(void * p_context)
{
    uint32_t        err_code;
    ble_cps_cycling_power_measurement_t cps_masurement;

    UNUSED_PARAMETER(p_context);

    cps_sim_measurement(&cps_masurement);

    err_code = ble_cps_cycling_power_measurement_send(&m_cps, &cps_masurement);
    if ((err_code != NRF_SUCCESS) &&
        (err_code != NRF_ERROR_INVALID_STATE) &&
        (err_code != BLE_ERROR_NO_TX_BUFFERS) &&
        (err_code != BLE_ERROR_GATTS_SYS_ATTR_MISSING)
...
static void timers_init(void)
{
    ...
    err_code = app_timer_create(&m_cps_meas_timer_id,
                                APP_TIMER_MODE_REPEATED,
                                cps_meas_timeout_handler);
    APP_ERROR_CHECK(err_code);
}
...
static void application_timers_start(void)
{
    ...
    //APP_ERROR_CHECK(err_code);
	  ...
	  err_code = app_timer_start(m_cps_meas_timer_id, POWER_MEAS_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
...
static void conn_params_init(void)
{
    ...
    connection_params_init.max_conn_params_update_count   = MAX_CONN_PARAMS_UPDATE_COUNT;
	  ...
	  connection_params_init.start_on_notify_cccd_handle    =  m_cps.cycling_power_measurement_handles.cccd_handle;
	//m_cscs.meas_handles.cccd_handle;
